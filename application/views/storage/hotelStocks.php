...
<script type="text/javascript">	
	...
	$(document).ready(function() {
		...
		itemTable = $("#stockItems").DataTable({
			...
			'serverside':true,
			'ajax': {
				"url": "<?php echo site_url('admin/fetchHotelStock')?>",
				"type": "POST"
			},
		...
		$('#btnLoad').click(function(){
			...
			$('#loadForm').attr('action','<?php echo base_url("admin/loadHotelItem")?>');
		});
		...
		$('#bulkDelete').click(function(){
			...
			$('#loadForm2').attr('action','<?php echo base_url("admin/bulkDeleteHotel")?>');
		});
		...
		$('#stockItems').on('click','.item-edit',function(){
			...
			$('#myForm').attr('action','<?php echo base_url("admin/updateHotelItem")?>');
			$.ajax({
				type: 'ajax',
				method: 'get',
				url: '<?php echo base_url("admin/editHotelItem")?>',
				data: {id: id},
				async: false,
				dataType: 'json',
				success: function(data){
					$('select[name=category]').val(data.stockCat_id);
					$('input[name=name]').val(data.hotelitem_name);
					$('input[name=unit]').val(data.hotelitem_unit);
					$('input[name=cost]').val(data.hotelitem_cost);
					$('input[name=qty]').val(data.hotelitem_stock);
					$('input[name=used]').val(data.hotelitem_used);
					$('input[name=itemId]').val(data.hotelitem_id);
				},
				error: function(){
		...
		$('#stockItems').on('click','.item-delete',function(){
			...
			$('#btnDelete').unbind().click(function(){
				$.ajax({
					...
					method: 'get',
					async:true,
					url: '<?php echo base_url("admin/deleteHotelItem")?>',
					data: {id: id},
					dataType: 'json',
					success: function(response){
						if (response.success) {
		...
		function showHotelCategory(){
			$.ajax({
				url:'<?php echo base_url("admin/fetchHotelCat")?>',
				async:false,
				dataType:'json',
				success: function(data){
					var html = '';
		...
		manualItemTable = $("#manualItems").DataTable({
			...
			'serverside':true,
			'ajax': {
				"url": "<?php echo site_url('admin/getManualHotelItem')?>",
				"type": "POST"
			},
            "dom": '<"top"l>rt<"bottom"ip><"clear">',
            'bProcessing': false,
	    ...
	    $('#manualItems').on('click','.item-add',function(){
			...
			$.ajax({
				...
				method: 'get',
				async:true,
				url: '<?php echo base_url("admin/manualAddHotel")?>',
				data: {id: id},
				dataType: 'json',
				success: function(response){
					if (response.success) {
