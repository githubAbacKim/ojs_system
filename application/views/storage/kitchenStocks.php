...
<script type="text/javascript">	
	...
	$(document).ready(function() {
		...
		itemTable = $("#stockItems").DataTable({
			...
			'serverside':true,
			'ajax': {
				"url": "<?php echo site_url('admin/fetchKitchenItem')?>",
				"type": "POST"
			},
		...
		$('#btnLoad').click(function(){
			...
			$('#loadForm').attr('action','<?php echo base_url("admin/loadKitchenItem")?>');
		});
		...
		$('#bulkDelete').click(function(){
			...
			$('#loadForm2').attr('action','<?php echo base_url("admin/bulkDeleteKitchen")?>');
		});
				...
				$.ajax({
					...
					success: function(response){
						if (response.success) {
							...
							$('.alert-success').html('Kitchen Items Deleted Successfully').fadeIn().delay(1000).fadeOut('slow');
							itemTable.ajax.reload(null, false);
							showKitchenCategory();
							showCategory();
						}else{
		...
		$('#stockItems').on('click','.item-edit',function(){
			...
			$('#myForm').attr('action','<?php echo base_url("admin/updateKitchenItem")?>');
			$.ajax({
				type: 'ajax',
				method: 'get',
				url: '<?php echo base_url("admin/editKitchenItem")?>',
				data: {id: id},
				async: true,
				dataType: 'json',
				success: function(data){
					$('select[name=category]').val(data.stockCat_id);
					$('input[name=name]').val(data.kitchenitem_name);
					$('input[name=unit]').val(data.kitchenitem_unit);
					$('input[name=cost]').val(data.kitchenitem_cost);
					$('input[name=qty]').val(data.kitchenitem_stock);
					$('input[name=dispose]').val(data.kitchenitem_dispose);
					$('input[name=itemId]').val(data.kitchenitem_id);
				},
				error: function(){
		...
		$('#stockItems').on('click','.item-delete',function(){
			...
			$('#btnDelete').unbind().click(function(){
				$.ajax({
					...
					method: 'get',
					async:true,
					url: '<?php echo base_url("admin/deleteKitchenItem")?>',
					data: {id: id},
					dataType: 'json',
					success: function(response){
						if (response.success) {
		...
		function showKitchenCategory(){
			$.ajax({
				url:'<?php echo base_url("admin/fetchKitchenCat")?>',
				async:false,
				dataType:'json',
				success: function(data){
					var html = '';
		...
		function showCategory(){
			$.ajax({
				url:'<?php echo base_url("admin/fetchCategoryKit")?>',
				async:false,
				dataType:'json',
				success: function(data){
					var html = '';
		...
		manualItemTable = $("#manualItems").DataTable({
			...
			'serverside':true,
			'ajax': {
				"url": "<?php echo site_url('admin/getManualKitchenItem')?>",
				"type": "POST"
			},
            "dom": '<"top"l>rt<"bottom"ip><"clear">',
            'bProcessing': false,
	    ...
	    $('#manualItems').on('click','.item-add',function(){
			...
			$.ajax({
				...
				method: 'get',
				async:true,
				url: '<?php echo base_url("admin/manualAddKitchen")?>',
				data: {id: id},
				dataType: 'json',
				success: function(response){
					if (response.success) {
