<div class="row">
	...
	<div class="col-lg-12">		
        <table class="table table-striped table-bordered table-hover" id="expGuide">
        	<thead>
                <tr>
                    <th>Category</th>
                    <th>Description</th>
	      		...
	      		<div class="row">
	      			<div class="col-md-12">
						...
						<div class="form-group col-md-8">
							<label for="note">*Note (Contained item.)</label>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
	    ...
	    $('#expGuide thead th').each( function () {
	        var title = $(this).text();
	        $(this).html( '<input style="width:100%;font-size:12px;" type="text" placeholder="'+title+'" />' );
	    } );

		expGuideTable = $("#expGuide").DataTable({
			'processing':true,
			'serverside':true,
			'ajax': {
				"url": "<?php echo site_url('admin/fetchExpGuide')?>",
				"type": "POST"
			},
		...
		// Apply the search
	    expGuideTable.columns().every( function () {
	        var that = this;
		...
		$('#btnAdd').click(function(){
			$('#myForm')[0].reset();
			$('#myModal').modal('show');
			$('#myModal').find('.modal-title').text("Add Production Expenses Guide");
			$('#myForm').attr('action','<?php echo base_url("admin/add_ExpGuide")?>');
		});
			...
			$.ajax({
	            ...
	            success: function(response){
	                ...
	                if (response.success) {
	                    ...
	                    expGuideTable.ajax.reload(null, false);
		...
		//edit
		$('#expGuide').on('click','.item-edit',function(){
			var id =  $(this).attr('data');
			$('#myModal').modal('show');
			$('#myModal').find('.modal-title').text('Edit Production Expense');
			$('#myForm').attr('action','<?php echo base_url("admin/updateExpEquip")?>');
			$.ajax({
				type: 'ajax',
				method: 'get',
				url: '<?php echo base_url("admin/editExpEquip")?>',
				data: {id: id},
				async: true,
				dataType: 'json',
				success: function(data){
					$('input[name=id]').val(data.expequip_id);
					$('select[name=mon]').val(data.expequip_mon);
					$('input[name=desc]').val(data.expequip_desc);
					$('input[name=unit]').val(data.expequip_unit);
					$('input[name=cost]').val(data.expequip_price);
					$('input[name=qty]').val(data.expequip_qty);
					$('input[name=date]').val(data.expequip_date);
					$('textarea[name=note]').val(data.expequip_note);
				},
				error: function(){
		...
		$('#expGuide').on('click','.item-delete',function(){
			var id =  $(this).attr('data');
			...
			$('#btnDelete').unbind().click(function(){
				$.ajax({
					...
					method: 'get',
					async:true,
					url: '<?php echo base_url("admin/deleteExpEquip")?>',
					data: {id: id},
					dataType: 'json',
					success: function(response){
						if (response.success) {
		...
		$('#btnPrint').click(function(){
            ...
            $('#printModal').find('.modal-title').text("Print Equipment Expenses Monthly");
	    ...
	    $('#conPrint').click(function(){
			...
			var url = '<?php echo base_url('admin/printEquipList')?>/'  + month.val() + '/' + year.val();
