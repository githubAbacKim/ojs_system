<div class="col-lg-12">
  <div class="row">
    <div class="col-lg-12">
        <h2 class="page-header"><i class="fa fa-money fa-fw"></i> Miscellanous Expenses</h2>
    </div>
  </div>
	<div class="row">
	...
	<div class="col-lg-12">
        <table class="table table-striped table-bordered table-hover" id="miscItem">
        	<thead>
                <tr>
                    <th>Date</th>
                    <th>Description</th>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
	    ...
	    $('#miscItem thead th').each( function () {
	        var title = $(this).text();
	        $(this).html( '<input style="width:100%;font-size:12px;" type="text" placeholder="'+title+'" />' );
	    } );

		miscTable = $("#miscItem").DataTable({
			'processing':true,
			'serverside':true,
			'ajax': {
				"url": "<?php echo site_url('admin/fetchMisc')?>",
				"type": "POST"
			},
		...
		// Apply the search
	    miscTable.columns().every( function () {
	        var that = this;
		...
		$('#miscItem').on('click','.item-edit',function(){
			...
			$.ajax({
				type: 'ajax',
				method: 'get',
				url: '<?php echo base_url("admin/editMisc")?>',
				data: {id: id},
				async: true,
				dataType: 'json',
				success: function(data){
					...
					$('input[name=qty]').val(data.misc_qty);
					$('input[name=date]').val(data.misc_date);
					$('textarea[name=note]').val(data.misc_note);
				},
				error: function(){
		...
		$('#miscItem').on('click','.item-delete',function(){
			var id =  $(this).attr('data');
			...
			$('#btnDelete').unbind().click(function(){
				$.ajax({
					...
					method: 'get',
					async:true,
					url: '<?php echo base_url("admin/deleteMisc")?>',
					data: {id: id},
					dataType: 'json',
					success: function(response){
						if (response.success) {
							$('#deleteModal').modal('hide');
							$('.alert-success').html('Successfully deleted miscellaneous.').fadeIn().delay(1000).fadeOut('slow');
							miscTable.ajax.reload(null, false);
						}else{
							alert('Error');
						}
  ...
  $('#conPrint').click(function(){
      ...
      var start = $('input[name=sdate]');
	    ...
	    var url = '<?php echo base_url('admin/printMiscList')?>/'  + start.val() + '/' + end.val();
