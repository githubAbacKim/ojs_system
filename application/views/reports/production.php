<div class="col-lg-12">
  <div class="row">
	...
	<div class="col-lg-12">
        <table class="table table-striped table-bordered table-hover" id="prodTable">
        	<thead>
                <tr>
              ...
              <form id="printForm2" action="" method="post">
                  <div class="col-md-6">
                      <fieldset>
                          ...
                          <div class="form-group">
                            <div class="form-group">
                                <input class="form-control" placeholder="Date" name="sdate2" value="<?php echo date('Y-m-d')?>" type="date" required autofocus />
                            </div>
                          </div>
                      </fieldset>
                  </div>
                  <div class="col-md-6">
                      <fieldset>
                          ...
                          <div class="form-group">
                            <div class="form-group">
                                <input class="form-control" placeholder="Date" name="edate2" value="<?php echo date('Y-m-d')?>" type="date" required autofocus />
                            </div>
                          </div>
                      </fieldset>
                  </div>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		prodTable = $("#prodTable").DataTable({
			...
			'serverside':true,
			'ajax': {
				"url": "<?php echo site_url('admin/fetchExpProd')?>",
				"type": "POST"
			},
		...
		/*$('#btnAdd').click(function(){
			$('#myForm')[0].reset();
			$('#myModal').modal('show');
			$('#myModal').find('.modal-title').text("Add Production Exp");
			$('#myForm').attr('action','<?php echo base_url("admin/addExpProd")?>');
		});*/
		...
		//edit
		$('#prodTable').on('click','.item-edit',function(){
			var id =  $(this).attr('data');
			$('#myModal').modal('show');
			$('#myModal').find('.modal-title').text('Edit Production Expense');
			$('#myForm').attr('action','<?php echo base_url("admin/updateExpProd")?>');
			$.ajax({
				type: 'ajax',
				method: 'get',
				url: '<?php echo base_url("admin/editExpProd")?>',
				data: {id: id},
				async: true,
				dataType: 'json',
				success: function(data){
					$('input[name=id]').val(data.prod_id);
					$('select[name=mon]').val(data.prod_mon);
					$('input[name=desc]').val(data.prod_desc);
					$('input[name=unit]').val(data.prod_unit);
					$('input[name=cost]').val(data.prod_price);
					$('input[name=qty]').val(data.prod_qty);
					$('input[name=date]').val(data.prod_date);
					$('textarea[name=note]').val(data.prod_note);
				},
				error: function(){
		...
		$('#prodTable').on('click','.item-delete',function(){
			var id =  $(this).attr('data');
			...
			$('#btnDelete').unbind().click(function(){
				$.ajax({
					...
					method: 'get',
					async:true,
					url: '<?php echo base_url("admin/deleteExpProd")?>',
					data: {id: id},
					dataType: 'json',
					success: function(response){
						if (response.success) {
    ...
    $('#conPrint').click(function(){
        ...
        var start = $('input[name=sdate]');
		    ...
		    var url = '<?php echo base_url('admin/printProdList')?>/'  + start.val() + '/' + end.val();
    ...
    $('#conPrint2').click(function(){
        ...
        var start2 = $('input[name=sdate2]');
		    var end2 = $('input[name=edate2]');
		    var url = '<?php echo base_url('admin/printProdTPList')?>/'  + start2.val() + '/' + end2.val() + '/' + 'finished';
        window.open(url,"newwindow", "width=1270, height=720");
    });
	});
