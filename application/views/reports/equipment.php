<div class="col-lg-12">
  <div class="row">
    <div class="col-lg-12">
        <h2 class="page-header"><i class="fa fa-money fa-fw"></i> Equipment Expenses</h2>
    </div>
  </div>
	<div class="row">
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		setInterval(function(){
            equipTable.ajax.reload(null, false);
        },3000);
		// Setup - add a text input to each footer cell
	    $('#expEquipItem thead th').each( function () {
	        var title = $(this).text();
	        $(this).html( '<input style="width:100%;font-size:12px;" type="text" placeholder="'+title+'" />' );
	    } );

		equipTable = $("#expEquipItem").DataTable({
			'processing':true,
			'serverside':true,
			'ajax': {
				"url": "<?php echo site_url('admin/fetchExpEquip')?>",
				"type": "POST"
			},
		...
		// Apply the search
	    equipTable.columns().every( function () {
	        var that = this;
		...
		$('#btnAdd').click(function(){
			...
			$('#myForm').attr('action','<?php echo base_url("admin/addExpEquip")?>');
		});
		...
		//edit
		$('#expEquipItem').on('click','.item-edit',function(){
		...
		$('#expEquipItem').on('click','.item-delete',function(){
			var id =  $(this).attr('data');
